...
async def start_money(message: types.Message):
    ...
    if  look_user_get:
        ...
        if subs:
            ...
            web_subs = [sub for sub in subs if sub["keyword"] == "Обмен валют"]
            if not web_subs:
                ...
                if end_date < now:
                    ...
                    async def new_message(message: types.Message):
                        ...
                        if message.chat.id in agent_id:
                            ...
                            unique_users = set(user['user_id'] for user in subs if user['keyword'] == 'Обмен валют')
                            ...
                            for sub in unique_users:
                                ...
                                if not web_subs:
                                    ...
                                    end_date = datetime.datetime.fromisoformat(web_sub["end_date"].replace('Z', '+00:00')).replace(tzinfo=pytz.UTC)
                                    now = datetime.datetime.now(pytz.UTC)
                                    users = web_sub['user_id']
                                    if end_date < now:
                                        continue
                                    else:
                                        
                                        for kw in get_kw:
                                            if kw['name'] == 'Обмен валют':
